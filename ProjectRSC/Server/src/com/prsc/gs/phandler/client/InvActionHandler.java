...
public class InvActionHandler implements PacketHandler {
	...
	if (player == null || player.getInventory() == null) {
        ...
        //Services.lookup(DatabaseManager.class).addQuery(new GenericLog(player.getUsername() + " used item " + item + " at " + player.getLocation()));
            ...
            World.getWorld().getDelayedEventHandler().add(new MiniEvent(player) {
                public void action() {
                    ...
                    switch (item.getID()) {
                        case 20: // Bones
                        case 604: // Bat bones
                            owner.incExp(5, 4, true);
                            break;
                        case 413: // Big bones
                            owner.incExp(5, 15, true);
                            break;
                        case 814: // Dragon bones
                            owner.incExp(5, 72, true);
                            break;
                    }
                        ...
                        public void action() {
                            ...
                            owner.setMenuHandler(new MenuHandler(options) {
                                public void handleReply(final int option, final String reply) {
                                    ...
                                    if (owner.getLocation().wildernessLevel() >= 30 || (owner.getLocation().inModRoom() && !owner.isAdmin())) {
