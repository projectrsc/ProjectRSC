package com.prsc.gs.model.component.mob;
...
public class Scriptable {
	...
	public int pickOption(String[] strs, boolean repeat) {
		try {
			long time = GameEngine.getAccurateTimestamp();
			...
			while (player.lastOption == -2 && GameEngine.getAccurateTimestamp() - time < 20000) { // timeout
				sleep(12);
			}
	...
	public Npc closestNpc(int npcId, int radius) {
		Iterable<Npc> npcsInView = Area.getViewableNpcs(player.getLocation(), radius);
		Npc closestNpc = null;
		for(int next = radius; next >= radius; next--) {
			for(Npc n : npcsInView) {
				if(n.getID() == npcId && n.withinRange(player.getLocation(), next)) {
					closestNpc = n;
				}
			}
		}
	...
	public boolean objectExists(GameObject object) {
		return false;//World.getWorld().getTile(object.getX(), object.getY()).hasGameObject();
	}
	//FIX
	public GameObject createNewObject(GameObject object) {
		return null; //World.getWorld().getTile(object.getX(), object.getY()).getGameObject();
	}
