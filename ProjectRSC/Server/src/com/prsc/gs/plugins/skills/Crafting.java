...
import com.prsc.gs.external.ItemCraftingDef;
import com.prsc.gs.external.ItemGemDef;
import com.prsc.gs.model.*;
import com.prsc.gs.plugins.listeners.action.InvUseOnItemListener;
            ...
            case 813: // Furnace
                        ...
                        public void action() {
                            ...
                            owner.setMenuHandler(new MenuHandler(options) {
                                public void handleReply(int option, String reply) {
                                    ...
                                    String[] options = {"Gold", "Sapphire", "Emerald", "Ruby", "Diamond"};
                                    ...
                                    owner.setMenuHandler(new MenuHandler(options) {
                                        public void handleReply(int option, String reply) {
                                            if (owner.isBusy() || option < 0 || option > 4) {
                                                return;
                                            }
                ...
                if (item.getID() == 384) { // Silver Bar (Crafting)
                    world.getDelayedEventHandler().add(new MiniEvent(owner) {
                        public void action() {
                            owner.getActionSender().sendMessage("What would you like to make?");
                            String[] options = new String[]{"Holy Symbol of Saradomin"};
                            owner.setMenuHandler(new MenuHandler(options) {
                                public void handleReply(int option, String reply) {
