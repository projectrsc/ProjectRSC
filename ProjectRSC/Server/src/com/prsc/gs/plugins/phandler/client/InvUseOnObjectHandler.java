...
public class InvUseOnObjectHandler implements PacketHandler {
    /**
     * World instance
     */
    public static final World world = World.getWorld();
    
    //final ActiveTile tile
    private void handleDoor(final Player player, final GameObject object, final int dir, final InvItem item) {
        ...
        World.getWorld().getDelayedEventHandler().add(new WalkToObjectEvent(player, object, false) {
            public void arrived() {
                owner.resetPath(); //|| !tile.hasGameObject() || !tile.getGameObject().equals(object)
                if (owner.isBusy() || owner.isRanging() || !owner.getInventory().contains(item) || owner.getStatus() != Action.USING_INVITEM_ON_DOOR) {
                	return;
                }
                owner.resetAll();
    ...
    private void handleObject(final Player player, final GameObject object, final InvItem item) {
        ...
        World.getWorld().getDelayedEventHandler().add(new WalkToObjectEvent(player, object, false) {
            public void arrived() {
                ...
                if (owner.isBusy() || owner.isRanging() || !owner.getInventory().contains(item) || !owner.withinRange(object, 1) || owner.getStatus() != Action.USING_INVITEM_ON_OBJECT) {
                	return;
                }
                owner.resetAll();
                                ...
                                if (owner.getInventory().remove(item) > -1) {
                                    ...
                                    for (InvItem i : loot) {
                                        if (i.getAmount() > 1 && !i.getDef().isStackable()) {
                                            ...
                                            Logger.println("Player: " + owner.getUsername() + " Got item: " + i.getID() + " From CHEST (" + i.getAmount() + ") sys time (" + GameEngine.getAccurateTimestamp() + ")");
                                            if (i.getAmount() > 4000) {
                                                Logger.println("WARNING!!!! Player: " + owner.getUsername() + " was about to get " + i.getAmount() + " of " + i.getID() + " from the CHEST sys time (" + GameEngine.getAccurateTimestamp() + ")");
                                                owner.setBusy(false);
                                                owner.getActionSender().sendInventory();
                                                return;
                                            }
    ...
    public void handlePacket(Packet p, Channel channel) throws Exception {
        ...
        int x = p.readShort();
        int y = p.readShort();
        GameObject object = player.getCurrentArea().getObject(x, y);//tile.getGameObject();
        InvItem item;
        switch (pID) {
            case 36: // Use Item on Door
                ...
                handleDoor(player, object, dir, item); //tile
                break;
            case 94: // Use Item on GameObject
                ...
                handleObject(player, object, item); // tile
                break;
        }
        //tile.cleanItself();
    }
