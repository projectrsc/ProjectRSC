...
public class AttackHandler implements PacketHandler {
	...
	public void handlePacket(Packet p, Channel channel) throws Exception {
		...
		if (affectedMob instanceof Player) {
			...
			if (pl.getLocation().inWilderness() && GameEngine.getAccurateTimestamp() - pl.getLastRun() < 3000) {
				return;
			}
		...
		if (player.getRangeEquip() < 0) {
			World.getWorld().getDelayedEventHandler().add(new WalkToMobEvent(player, affectedMob, affectedMob instanceof Npc ? 1 : 2) {
				public void arrived() {
					...
					if (owner.isBusy() || affectedMob.isBusy() || !owner.withinRange(affectedMob, affectedMob instanceof Npc ? 1 : 2) || !owner.checkAttack(affectedMob, false) || owner.getStatus() != Action.ATTACKING_MOB) {
						return;
					}
