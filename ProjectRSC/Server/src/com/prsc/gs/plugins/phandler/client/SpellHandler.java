...
import com.prsc.gs.event.impl.ObjectRemover;
...
import com.prsc.gs.external.ReqOreDef;
import com.prsc.gs.external.SpellDef;
...
import com.prsc.gs.model.Projectile;
...
public class SpellHandler implements PacketHandler {
    ...
    public void handlePacket(Packet p, Channel channel) throws Exception {
        ...
        if (spell.isMembers() && !Constants.GameServer.MEMBER_WORLD) {
            player.getActionSender().sendMessage("You need to login to a members world to use this spell");
            player.resetPath();
            return;
        }

        //Services.lookup(DatabaseManager.class).addQuery(new GenericLog(player.getUsername() + " tried to cast spell 49 at " + player.getLocation()));
    ...
    private void finalizeSpell(Player player, SpellDef spell) {
        player.lastCast = GameEngine.getAccurateTimestamp();
    ...
    private void handleItemCast(Player player, final SpellDef spell, final int id, final Item affectedItem) {
        ...
        World.getWorld().getDelayedEventHandler().add(new WalkToPointEvent(player, affectedItem.getLocation(), 5, true) {
            public void arrived() {
                owner.resetPath();
               // ActiveTile tile = world.getTile(location); //|| !tile.hasItem(affectedItem)
                if (!canCast(owner) || owner.getStatus() != Action.CASTING_GITEM || affectedItem.isRemoved()) {
                	System.out.println("doode");
                    return;
                }
    ...
    private void handleMobCast(final Player player, Mob affectedMob, final int spellID) {
        ...
        if (affectedMob instanceof Player) {
            ...
            if (player.getLocation().inWilderness() && GameEngine.getAccurateTimestamp() - other.getLastRun() < 1000) {
                player.resetPath();
                return;
            }
        }
        if (player.getLocation().inWilderness() && GameEngine.getAccurateTimestamp() - player.getLastRun() < 3000) {
            player.resetPath();
            return;
        }
            ...
            public void arrived() {
                ...
                switch (spellID) {
                    ...
                    case 9: // Curse
                        ...
                        playersToInformm.addAll(Lists.newArrayList(owner.getViewArea().getPlayersInView()));
                        playersToInformm.addAll(Lists.newArrayList(affectedMob.getViewArea().getPlayersInView()));
                    ...
                    case 19: // Crumble undead
                        ...
                        playersToInforms.addAll(Lists.newArrayList(owner.getViewArea().getPlayersInView()));
                        playersToInforms.addAll(Lists.newArrayList(affectedMob.getViewArea().getPlayersInView()));
                    ...
                    case 25:
                        ...
                        if (charged) {
                            ...
                            playersToInfor.addAll(Lists.newArrayList(owner.getViewArea().getPlayersInView()));
                            playersToInfor.addAll(Lists.newArrayList(affectedMob.getViewArea().getPlayersInView()));
    ...
    private void handleTeleport(Player player, SpellDef spell, int id) {
        if (player.getLocation().wildernessLevel() >= 10 || (player.getLocation().inModRoom() && !player.isAdmin())) {
	...
	public int[] getAssociatedIdentifiers() {
		return new int[]{206, 55, 71, 49, 67, 17, 104, 232};
	}
