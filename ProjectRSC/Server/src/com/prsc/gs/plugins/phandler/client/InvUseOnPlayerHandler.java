...
public class InvUseOnPlayerHandler implements PacketHandler {
    /**
     * World instance
     */
    ...
    public void handlePacket(Packet p, Channel channel) throws Exception {
        ...
        if (GameEngine.getAccurateTimestamp() - affectedPlayer.getLastRun() < 2000) {
            player.resetPath();
            return;
        }
        ...
        World.getWorld().getDelayedEventHandler().add(new WalkToMobEvent(player, affectedPlayer, 1) {
            public void arrived() {
                owner.resetPath();
                owner.resetFollowing(); //!owner.nextTo(affectedPlayer)
                if (!owner.getInventory().contains(item) || !owner.withinRange(affectedPlayer, 1) || owner.isBusy() || owner.isRanging() || owner.getStatus() != Action.USING_INVITEM_ON_PLAYER) {
                    return;
                }
                owner.resetAll();
