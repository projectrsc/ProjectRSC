...
public class DuelHandler implements PacketHandler {
    ...
    public void handlePacket(Packet p, Channel channel) throws Exception {
        ...
        if (!Constants.GameServer.MEMBER_WORLD) {
            player.getActionSender().sendMessage("You need to be a member to use this feature");
            return;
        }
        ...
        switch (pID) {
            case 222: // Sending duel request
                ...
                if (!player.isDueling() && affectedPlayer.getWishToDuel() != null && affectedPlayer.getWishToDuel().equals(player) && !affectedPlayer.isDueling()) {
                    ...
                    //Services.lookup(DatabaseManager.class).addQuery(new GenericLog(player.getUsername() + " sent duel request to " + affectedPlayer.getUsername() + " at " + player.getLocation()));
                }
                break;
            case 252: // Duel accepted
                ...
                //Services.lookup(DatabaseManager.class).addQuery(new GenericLog(player.getUsername() + " acceptet duel request with " + affectedPlayer.getUsername() + " at " + player.getLocation()));
                break;
            case 87: // Confirm accepted
                ...
                if (affectedPlayer.isDuelConfirmAccepted()) {
                        ...
                        public void arrived() {
                            World.getWorld().getDelayedEventHandler().add(new SingleEvent(owner, 300) {
                                public void action() {
                                    ...
                                    owner.resetPath(); //!owner.nextTo(affectedPlayer)
                                    if (!owner.withinRange(affectedPlayer, 1)) {
                                        unsetOptions(owner);
                                        unsetOptions(affectedPlayer);
                                        return;
                                    }
                    ...
                    walking.setLastRun(GameEngine.getAccurateTimestamp() + 500);
            ...
            case 35: // Decline duel
                ...
                unsetOptions(player);
                unsetOptions(affectedPlayer);
                //Services.lookup(DatabaseManager.class).addQuery(new GenericLog(player.getUsername() + " declined duel with " + affectedPlayer.getUsername() + " at " + player.getLocation()));
                break;
	...
	public int[] getAssociatedIdentifiers() {
		return new int[]{35, 123, 87, 222, 225, 252};
	}
