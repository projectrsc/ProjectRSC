...
public class WalkRequestHandler implements PacketHandler {
	/**
	 * World instance
	 */
	...
	public void handlePacket(Packet p, Channel channel) throws Exception {
		try {
			...
			if (player.inCombat()) {
				if (pID == 132) {
					...
					if (opponent.getHitsMade() >= 3) {
						...
						player.setLastRun(GameEngine.getAccurateTimestamp()); //System.nanoTime() / 1000000
			...
			if (GameEngine.getAccurateTimestamp() - player.lastCast < 600)
				return;
