package org.rscangel.client.camera;
...
public final class EngineHandle {
	...
	private final JagGraphics gameImage;

	//jag landscape variables
	public byte[] landscapeFree, mapsFree;
	public byte[] landscapeMembers, mapsMembers;

	private final byte[][] tileGroundElevation;
	private final byte[][] tileRoofType;
	private final byte[][] tileGroundTexture;
	private final int[][] tileDiagonalWall;
	private final byte[][] tileGroundOverlay;
	private final byte[][] tileObjectRotation;
	private final byte[][] tileHorizontalWall;
	private final byte[][] tileVerticalWall;
	//end of vars

	private final int[] tileColors;

	public EngineHandle(Camera camera, JagGraphics gameImage) {
		this.camera = camera;
		...
		tileColors = new int[256];
		tiles = new int[96][96];
		playerIsAlive = false;

		for (int i = 0; i < 64; i++) {
			tileColors[i] = Camera.method305(255 - i * 4, 255 - (int) ((double) i * 1.75D), 255 - i * 4);
		}
		for (int j = 0; j < 64; j++) {
			tileColors[j + 64] = Camera.method305(j * 3, 144, 0);
		}
		for (int k = 0; k < 64; k++) {
			tileColors[k + 128] = Camera.method305(192 - (int) ((double) k * 1.5D), 144 - (int) ((double) k * 1.5D), 0);
		}
		for (int l = 0; l < 64; l++) {
			tileColors[l + 192] = Camera.method305(96 - (int) ((double) l * 1.5D), 48 + (int) ((double) l * 1.5D), 0);
		}
	}

	public int generatePath(int curX, int curY, int bottomDestX, int bottomDestY, int upperDestX, int upperDestY, int[] pathX, int[] pathY, boolean useObjects) {
		int[][] steps = new int[96][96];
		...
		while (stepCount != requiredSteps) {
			...
			if (useObjects) {
				...
				if (x < 95 && x + 1 >= bottomDestX && x + 1 <= upperDestX && y >= bottomDestY
						&& y <= upperDestY && (tiles[x + 1][y] & 2) == 0) {
					flag1 = true;
					break;
				}
				if (y > 0 && x >= bottomDestX && x <= upperDestX && y - 1 >= bottomDestY
						&& y - 1 <= upperDestY && (tiles[x][y - 1] & 4) == 0) {
					flag1 = true;
					break;
				}
				if (y < 95 && x >= bottomDestX && x <= upperDestX && y + 1 >= bottomDestY
						&& y + 1 <= upperDestY && (tiles[x][y + 1] & 1) == 0) {
					flag1 = true;
					break;
				}
			}
			if (x > 0 && steps[x - 1][y] == 0
					&& (tiles[x - 1][y] & 0x78) == 0) {
			...
			if (x > 0 && y > 0 && (tiles[x][y - 1] & 0x74) == 0
					&& (tiles[x - 1][y] & 0x78) == 0
					&& (tiles[x - 1][y - 1] & 0x7c) == 0
					&& steps[x - 1][y - 1] == 0) {
				pathX[requiredSteps] = x - 1;
				pathY[requiredSteps] = y - 1;
				requiredSteps = (requiredSteps + 1) % i3;
			...
			if (x < 95 && y > 0 && (tiles[x][y - 1] & 0x74) == 0
					&& (tiles[x + 1][y] & 0x72) == 0
					&& (tiles[x + 1][y - 1] & 0x76) == 0
					&& steps[x + 1][y - 1] == 0) {
				pathX[requiredSteps] = x + 1;
				pathY[requiredSteps] = y - 1;
				requiredSteps = (requiredSteps + 1) % i3;
			...
			if (x > 0 && y < 95 && (tiles[x][y + 1] & 0x71) == 0
					&& (tiles[x - 1][y] & 0x78) == 0
					&& (tiles[x - 1][y + 1] & 0x79) == 0
					&& steps[x - 1][y + 1] == 0) {
				pathX[requiredSteps] = x - 1;
				pathY[requiredSteps] = y + 1;
				requiredSteps = (requiredSteps + 1) % i3;
		...
		pathY[stepCount++] = y;
		int k3;
		for (int j3 = k3 = steps[x][y]; x != curX
				|| y != curY; j3 = steps[x][y]) {
			if (j3 != k3) {
				k3 = j3;
				pathX[stepCount] = x;
	...
	public int getGroundElevation(int x, int y) {
		...
		return (tileGroundElevation[z][x * 48 + y] & 0xff) * 3;
	}
	...
	public void loadSector(int i, int j, int k) {
		garbageCollect();
		int l = (i + 24) / 48;
		int i1 = (j + 24) / 48;
	...
	public void setGroundTexturesOverlay(int x, int y, int k) {
		...
		tileGroundOverlay[z][x * 48 + y] = (byte) k;
	}
	...
	public void method409(int i, int j, int k, boolean render) {
		int l = (i + 24) / 48;
		int i1 = (j + 24) / 48;
		loadSection(l - 1, i1 - 1, k, 0);
		loadSection(l, i1 - 1, k, 1);
			...
			for (int j3 = 0; j3 < 95; j3++) {
				for (int j4 = 0; j4 < 95; j4++) {
					...
					if (getGroundTexturesOverlay(j3, j4) > 0) {
						...
						if (l5 == 5) {
							if (getDiagonalWall(j3, j4) > 0
									&& getDiagonalWall(j3, j4) < 24000)
						...
						} else if (l5 != 2 || getDiagonalWall(j3, j4) > 0
								&& getDiagonalWall(j3, j4) < 24000)
					...
					drawMinimapTile(j3, j4, l14, k7, i10);
		...
		for (int x = 0; x < 95; x++) {
			for (int y = 0; y < 95; y++) {
				...
				if (tileValue > 0 && EntityHandler.getDoorDef(tileValue - 1).getUnknown() == 0) {
					method421(aModel, tileValue - 1, x, y, x, y + 1);

					if (render && EntityHandler.getDoorDef(tileValue - 1).getDoorType() != 0) {
						tiles[x][y] |= 2;
						//tiles[x][y] |= 1;
						if (x > 0) {
							orWalkable(x - 1, y, 8);
					...
					if (render) {
						gameImage.drawLineY(x * 3, y * 3, 3, color);
					}
				}
				
				tileValue = getVerticalWall(x, y);
				if (tileValue > 0 && EntityHandler.getDoorDef(tileValue - 1).getUnknown() == 0) {
					method421(aModel, tileValue - 1, x, y, x + 1, y);
					...
					if (render) {
						gameImage.drawLineX(x * 3, y * 3, 3, color);
					}
				}
				
				tileValue = getDiagonalWall(x, y);
				if (tileValue > 0 && tileValue < 12000 && EntityHandler.getDoorDef(tileValue - 1).getUnknown() == 0) {
					method421(aModel, tileValue - 1, x, y, x + 1, y + 1);

					if (render && EntityHandler.getDoorDef(tileValue - 1).getDoorType() != 0) {
						tiles[x][y] |= 0x20;
					}
					if (render) {
						gameImage.setPixelColour(x * 3, y * 3, color);
						gameImage.setPixelColour(x * 3 + 1, y * 3 + 1, color);
						gameImage.setPixelColour(x * 3 + 2, y * 3 + 2, color);
					}
				}
				if (tileValue > 12000 && tileValue < 24000 && EntityHandler.getDoorDef(tileValue - 12001).getUnknown() == 0) {
					method421(aModel, tileValue - 12001, x + 1, y, x, y + 1);

					if (render && EntityHandler.getDoorDef(tileValue - 12001).getDoorType() != 0) {
						tiles[x][y] |= 0x10;
					}
					if (render) {
						gameImage.setPixelColour(x * 3 + 2, y * 3, color);
						gameImage.setPixelColour(x * 3 + 1, y * 3 + 1, color);
						gameImage.setPixelColour(x * 3, y * 3 + 2, color);
					}
				}
			}
		}
		...
		for (int i7 = 1; i7 < 95; i7++) {
			for (int k9 = 1; k9 < 95; k9++) {
				...
				if (i12 > 0) {
					...
					l27 = -l27;
					i28 = -i28;
					if (getDiagonalWall(i7, k9) > 12000
							&& getDiagonalWall(i7, k9) < 24000
					...
					} else if (getDiagonalWall(i7, k9) > 12000
							&& getDiagonalWall(i7, k9) < 24000
							&& getRoofTexture(i7 + 1, k9 + 1) == 0) {
						int ai9[] = new int[3];
					...
					} else if (getDiagonalWall(i7, k9) > 0
							&& getDiagonalWall(i7, k9) < 12000
							&& getRoofTexture(i7 + 1, k9 - 1) == 0) {
						int ai10[] = new int[3];
	...
	public void registerObject(int sectorX, int sectorY, int objectId, int objectDir) {
		if (sectorX < 0 || sectorY < 0 || sectorX >= 95 || sectorY >= 95) {
			return;
		}
		if (EntityHandler.getObjectDef(objectId).getType() == 1
			...
			if (objectDir == 0 || objectDir == 4) {
				i1 = EntityHandler.getObjectDef(objectId).getWidth();
				j1 = EntityHandler.getObjectDef(objectId).getHeight();
			} else {
				j1 = EntityHandler.getObjectDef(objectId).getWidth();
				i1 = EntityHandler.getObjectDef(objectId).getHeight();
			}
			for (int k1 = sectorX; k1 < sectorX + i1; k1++) {
				for (int l1 = sectorY; l1 < sectorY + j1; l1++)
					if (EntityHandler.getObjectDef(objectId).getType() == 1)
						tiles[k1][l1] |= 0x40;
					else if (objectDir == 0) {
						tiles[k1][l1] |= 2;
			...
			method407(sectorX, sectorY, i1, j1);
		}
	}

	public void drawMinimapTile(int x, int y, int type, int base1, int base2) {
		int xx = x * 3;
		int yy = y * 3;
		int color1 = camera.getBaseColor(base1);
		int color2 = camera.getBaseColor(base2);
		color1 = color1 >> 1 & 0x7f7f7f;
						color2 = color2 >> 1 & 0x7f7f7f;

					if (type == 0) {
						gameImage.drawLineX(xx, yy, 3, color1);
						gameImage.drawLineX(xx, yy + 1, 2, color1);
						gameImage.drawLineX(xx, yy + 2, 1, color1);
						gameImage.drawLineX(xx + 2, yy + 1, 1, color2);
						gameImage.drawLineX(xx + 1, yy + 2, 2, color2);
					} else if (type == 1) {
						gameImage.drawLineX(xx, yy, 3, color2);
						gameImage.drawLineX(xx + 1, yy + 1, 2, color2);
						gameImage.drawLineX(xx + 2, yy + 2, 1, color2);
						gameImage.drawLineX(xx, yy + 1, 1, color1);
						gameImage.drawLineX(xx, yy + 2, 2, color1);
					}
	}
	...
	public int getVerticalWall(int x, int y) {
		...
		return tileVerticalWall[z][x * 48 + y] & 0xff;
	}
	...
	public int getDiagonalWall(int x, int y) {
		...
		return tileDiagonalWall[z][x * 48 + y];
	}
	...
	public int getGroundTexturesOverlay(int x, int y) {
		...
		return tileGroundOverlay[z][x * 48 + y] & 0xff;
	}
	...
	public void method421(Model model, int id, int x, int y, int x2, int y2) {
		method419(x, y, 40);
		method419(x2, y2, 40);
		int j1 = EntityHandler.getDoorDef(id).getModelVar1();
		int k1 = EntityHandler.getDoorDef(id).getModelVar2();
		int l1 = EntityHandler.getDoorDef(id).getModelVar3();
		int sectorX = x * 128;
		int sectorY = y * 128;
		int sectorX2 = x2 * 128;
		int sectorY2 = y2 * 128;
		int i3 = model.method179(sectorX, -anIntArrayArray581[x][y], sectorY);
		int j3 = model.method179(sectorX, -anIntArrayArray581[x][y] - j1, sectorY);
		int k3 = model.method179(sectorX2, -anIntArrayArray581[x2][y2] - j1, sectorY2);
		int l3 = model.method179(sectorX2, -anIntArrayArray581[x2][y2], sectorY2);
		int i4 = model.method181(4, new int[] { i3, j3, k3, l3 }, k1, l1);
		
		if (EntityHandler.getDoorDef(id).getUnknown() == 5) {
			model.anIntArray258[i4] = 30000 + id;
		} else {
	...
	public int getGroundTexture(int x, int y) {
		...
		return tileGroundTexture[z][x * 48 + y] & 0xff;
	}
	...
	public int getHorizontalWall(int x, int y) {
		...
		return tileHorizontalWall[z][x * 48 + y] & 0xff;
	}
	...
	public void loadSectorObjects(Model[] models) {
		for (int sectorX = 0; sectorX < 94; sectorX++) {
			for (int sectorY = 0; sectorY < 94; sectorY++) {
				if (getDiagonalWall(sectorX, sectorY) > 48000 && getDiagonalWall(sectorX, sectorY) < 60000) {
					int id = getDiagonalWall(sectorX, sectorY) - 48001;
					int direction = objectDirs[sectorX][sectorY];
					int width;
					int height;
					if (direction == 0 || direction == 4) {
						width = EntityHandler.getObjectDef(id).getWidth();
						height = EntityHandler.getObjectDef(id).getHeight();
					} else {
						height = EntityHandler.getObjectDef(id).getWidth();
						width = EntityHandler.getObjectDef(id).getHeight();
					}
					registerObject(sectorX, sectorY, id, direction);
					Model model = models[EntityHandler.getObjectDef(id).modelID].method204(false, true, false, false);
					int mapX = ((sectorX + sectorX + width) * 128) / 2;
					int mapY = ((sectorY + sectorY + height) * 128) / 2;
					model.method190(mapX, -getAveragedElevation(mapX, mapY), mapY);
					model.method189(0, direction * 32, 0);
					camera.addModel(model);
					model.method185(48, 48, -50, -10, -50);
					if (width > 1 || height > 1) {
						for (int k2 = sectorX; k2 < sectorX + width; k2++) {
							for (int l2 = sectorY; l2 < sectorY + height; l2++) {
								if ((k2 > sectorX || l2 > sectorY) && getDiagonalWall(k2, l2) - 48001 == id) {
									int x = k2;
									int y = l2;
									byte z = 0;
									if (x >= 48 && y < 48) {
			...
			if(landscapeFree != null) {
				...
				if(data != null && data.length > 0) {
					...
					for(int tile = 0; tile < 2304;) {
						...
						if(value < 128) {
							tileGroundElevation[sector][tile++] = (byte)value;
							lastValue = value;
						}
						if(value >= 128) {
							for(int k4 = 0; k4 < value - 128; k4++) {
								tileGroundElevation[sector][tile++] = (byte) lastValue;
							}
						}
					}

					lastValue = 64;
					for(int sector_height = 0; sector_height < 48; sector_height++) {
						for(int sector_width = 0; sector_width < 48; sector_width++) {
							lastValue = tileGroundElevation[sector][sector_width * 48 + sector_height] + lastValue & 0x7f;
							tileGroundElevation[sector][sector_width * 48 + sector_height] = (byte)(lastValue * 2);
						}
					...
					for(int tile = 0; tile < 2304;) {
						int value = data[off++] & 0xff;
						if(value < 128) {
							tileGroundTexture[sector][tile++] = (byte) value;
							lastValue = value;
						}
						if(value >= 128) {
							for(int i7 = 0; i7 < value - 128; i7++) {
								tileGroundTexture[sector][tile++] = (byte)lastValue;
							}
						}
					}
					...
					for(int x = 0; x < 48; x++) {
						for(int y = 0; y < 48; y++) {
							lastValue = tileGroundTexture[sector][y * 48 + x] + lastValue & 0x7f;
							tileGroundTexture[sector][y * 48 + x] = (byte)(lastValue * 2);
						}
				...
				for(int tile = 0; tile < 2304; tile++) {
					int value = data[off++] & 0xff;
					if(value > 0) {
						tileDiagonalWall[sector][tile] = value + 12000;
					}
				}
				...
				for(int tile = 0; tile < 2304;) {
					int value = data[off++] & 0xff;
					if(value < 128) {
						tileRoofType[sector][tile++] = (byte)value;
					} else {
				...
				for(int tile = 0; tile < 2304;) {
					int value = data[off++] & 0xff;
					if(value < 128) {
						tileGroundOverlay[sector][tile++] = (byte) value;
						lastValue = value;
					} else {
						for(int i = 0; i < value - 128; i++) {
							tileGroundOverlay[sector][tile++] = (byte) lastValue;
						}
					}
				}
				...
				for(int tile = 0; tile < 2304;) {
					int value = data[off++] & 0xff;
					if(value < 128) {
						tileObjectRotation[sector][tile++] = (byte)value;
					} else {
				...
				if(data != null && data.length > 0) {
					...
					for(int tile = 0; tile < 2304;) {
						...
						if(value < 128)
							tileDiagonalWall[sector][tile++] = value + 48000;
						else
							tile += value - 128;
					}
					return;
	...
	public Model[][] aModelArrayArray580;
	int[][] anIntArrayArray581;
	public Model[] aModelArray596;
	public Model[][] aModelArrayArray598;
}