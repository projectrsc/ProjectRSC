package org.rscangel.client.filter;

/* Depreciated as of 10/23/2012

public final class ChatFilter {

	public static void addFilterData(Buffer fragments, Buffer badwords, Buffer hostwords, Buffer tldlist) {
		loadBadEnc(badwords);
		loadHostsEnc(hostwords);
		loadFragmentsEnc(fragments);
		loadTldList(tldlist);
	}

	public static void loadTldList(Buffer buffer) {
		int length = buffer.getInt();
		ffi = new char[length][];
		ffj = new int[length];
		for(int i = 0; i < length; i++) {
			ffj[i] = buffer.getByte();
			char ac[] = new char[buffer.getByte()];
			for(int k = 0; k < ac.length; k++) {
				ac[k] = (char)buffer.getByte();
			}
			ffi[i] = ac;
		}
	...
	public static void loadHostsEnc(Buffer buffer) {
		int i = buffer.getInt();
		ffg = new char[i][];
		ffh = new byte[i][][];
		fgc(buffer, ffg, ffh);
	...
	public static void loadFragmentsEnc(Buffer buffer) {
		ffd = new int[buffer.getInt()];
		for(int i = 0; i < ffd.length; i++) {
			ffd[i] = buffer.getShort();
		}
	}

	public static void fgc(Buffer buffer, char arg1[][], byte arg2[][][]) {
		for(int i = 0; i < arg1.length; i++) {
			char ac[] = new char[buffer.getByte()];
			for(int j = 0; j < ac.length; j++)
				ac[j] = (char)buffer.getByte();

			arg1[i] = ac;
			byte abyte0[][] = new byte[buffer.getInt()][2];
			for(int k = 0; k < abyte0.length; k++) {
				abyte0[k][0] = (byte)buffer.getByte();
				abyte0[k][1] = (byte)buffer.getByte();
			}
