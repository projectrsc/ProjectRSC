...
import org.rscangel.client.GameWindow;
import org.rscangel.client.mudclient;
import org.rscangel.client.model.ColorMutator;
import org.rscangel.client.model.Sprite;
import org.rscangel.client.util.DataConversions;

public class JagGraphics implements ImageProducer, ImageObserver {

	public Sprite[] sprites;
	private ZipFile spriteArchive;

	public JagGraphics(int width, int height, int k, Component component) {
		interlace = false;
		drawStringShadows = false;
		imageHeight = height;
		imageWidth = width;
		imageWidthUnused = gameWidth = width;
		imageHeightUnused = gameHeight = height;
		imagePixelArray = new int[width * height];
		mutator = new ColorMutator();
		mutator.start();
	...
	public synchronized void completePixels() {
		...
		//drawBoxAlpha(0, 0, gameWidth, gameHeight, 0xffffff, mutator.getOffset());
		imageConsumer.setPixels(0, 0, gameWidth, gameHeight, colourModel, imagePixelArray, 0, gameWidth);
	...
	public void drawPixels(int pixels[][], int drawx, int drawy, int width, int height) {
		for(int x = drawx; x < drawx + width; x++) {
			for(int y = drawy; y < drawy + height; y++) {
				this.imagePixelArray[x + y * gameWidth] = pixels[x - drawx][y - drawy];
			}
		}
	}
	...
	public void clearScreen() {
		...
		int k = 0;
		for (int l = -gameHeight; l < 0; l += 2) {
			for (int i1 = -gameWidth; i1 < 0; i1++)
				imagePixelArray[k++] = 0;
	...
	public void applyFilter() {
		this.drawBoxAlpha(0, 0, gameWidth, gameHeight, 0xffffff, mutator.getOffset());
	}
	...
	public void drawCenteredBoxEdge(int x, int y, int width, int height, int color) {
		drawBoxEdge(x - width / 2, y - height / 2, width, height, color);
	}

	public void drawBoxEdge(int x, int y, int width, int height, int color) {
		drawLineX(x, y, width, color);
		drawLineX(x, (y + height) - 1, width, color);
		drawLineY(x, y, height, color);
		drawLineY((x + width) - 1, y, height, color);
	}
	...
	public void drawLineY(int x1, int y1, int y2, int colour) {
		...
		for (int yPixel = 0; yPixel < y2; yPixel++)
			imagePixelArray[xPixel + yPixel * gameWidth] = colour;
	...
	public void setPixelColour(int x, int y, int colour) {
		...
		imagePixelArray[x + y * gameWidth] = colour;
	}

	public void fadePixels() {
		int k = gameWidth * gameHeight;
		for (int j = 0; j < k; j++) {
		...
		for (int k1 = k; k1 < k + i1; k1++) {
			for (int l1 = l; l1 < l + j1; l1++) {
				...
				for (int i3 = k1 - i; i3 <= k1 + i; i3++)
					if (i3 >= 0 && i3 < gameWidth) {
						for (int j3 = l1 - j; j3 <= l1 + j; j3++)
							if (j3 >= 0 && j3 < gameHeight) {
								int k3 = imagePixelArray[i3 + gameWidth
								                         * j3];
								i2 += k3 >> 16 & 0xff;
							j2 += k3 >> 8 & 0xff;
			k2 += k3 & 0xff;
			l2++;
				...
				imagePixelArray[k1 + gameWidth * l1] = (i2 / l2 << 16)
						+ (j2 / l2 << 8) + k2 / l2;
			}
	...
	public static int[] convertLongToRGB(int value) {
	...
	public void storeSpriteHoriz(int index, int startX, int startY, int width,
		...
		for (int x = startX; x < startX + width; x++) {
			for (int y = startY; y < startY + height; y++) {
				pixels[pixel++] = imagePixelArray[x + y * gameWidth];
			}
		}
	...
	public void drawTransparentPicture(int x, int y, int picture, int opacity) {
		...
		int k1 = 0;
		int l1 = sprites[picture].getHeight();
		int i2 = sprites[picture].getWidth();
		int j2 = gameWidth - i2;
		int k2 = 0;
		if(y < imageY) {
			int l2 = imageY - y;
		...
		if(interlace) {
			byte0 = 2;
			j2 += gameWidth;
			k2 += sprites[picture].getWidth();
			if((y & 1) != 0) {
				j1 += gameWidth;
				l1--;
			}
		}
		if (sprites[picture].getPixels() == null)
			cck(imagePixelArray, sprites[picture].getUnknownByteArray(), sprites[picture].getTransparentPixels(), k1, j1, i2, l1, j2, k2, byte0, opacity);
		else
			ccj(imagePixelArray, sprites[picture].getPixels(), 0, k1, j1, i2, l1, j2, k2, byte0, opacity);
	}
	...
	public void drawPicture(int x, int y, int picture) {
		try {
			...
			if (sprites[picture].getPixels() == null)
				cch(imagePixelArray, sprites[picture].getUnknownByteArray(), sprites[picture].getTransparentPixels(), i1, l, k1, j1, l1, i2, byte0);
			else
	...
	public void drawSprite(int i, int j, int k, int l, int i1) {
		try {
	...
	public void cbl(int spriteId) {
		...
		sprites[spriteId].setUnknownByteArray(abyte0);
	...
	public void drawVisibleEntity(int i, int j, int k, int l, int i1, int j1, int k1) {
		drawSprite(i, j, k, l, i1);
	}
	...
	public static int addFont(byte[] data) {
		gameFonts[currentFont] = data;
		return currentFont++;
	}
	...
	public int fontHeight(int messageType) {
		if (messageType == 0)
			return 12;
		if (messageType == 1)
			return 14;
	...
	private final ColorMutator mutator;
