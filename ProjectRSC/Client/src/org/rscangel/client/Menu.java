	...
	public void mouseClick(int mouseX, int mouseY, int lastMouseButton, int mouseButton) {
		...
		if (lastMouseButton == 1) {
			for (int menuObject = 0; menuObject < menuObjectCount; menuObject++) {
				if (componentAcceptsInput[menuObject]
						&& componentType[menuObject] == 10
						&& mouseX >= componentX[menuObject]
								&& mouseY >= componentY[menuObject]
										&& mouseX <= componentX[menuObject]
												+ componentWidth[menuObject]
														&& mouseY <= componentY[menuObject]
																+ componentHeight[menuObject])
					componentSkip[menuObject] = true;
				if (componentAcceptsInput[menuObject]
						&& componentType[menuObject] == 14
						&& mouseX >= componentX[menuObject]
								&& mouseY >= componentY[menuObject]
										&& mouseX <= componentX[menuObject]
												+ componentWidth[menuObject]
		...
		if (lastMouseButton == 1 || mouseClicksConsecutive > 20) {
			for (int j1 = 0; j1 < menuObjectCount; j1++)
				if (componentAcceptsInput[j1] && componentType[j1] == 15
				&& mouseX >= componentX[j1]
						&& mouseY >= componentY[j1]
								&& mouseX <= componentX[j1] + componentWidth[j1]
										&& mouseY <= componentY[j1] + componentHeight[j1])
