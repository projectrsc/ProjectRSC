...
public abstract class GameWindowMiddleMan extends GameWindow {
	...
	public final void login(String user, String pass, boolean reconnecting) {
		...
		try {
			...
			if(loginCode == -1) {
				...
				loginScreenPrint("You may only use 3 characters at once.", "Your ip-address is already in use");
				return;
			} else if(loginCode == 9) {
				...
				return;
			} else if (loginCode == 13) {
				loginScreenPrint("None of your characters can log in.", "Contact customer support");
				return;
			} else if (loginCode == 14) {
				loginScreenPrint("You need to set your display name.", "Please go to the Account Management page to do this.");
				return;
			} else if (loginCode == 15) {
				loginScreenPrint("Sorry! This world is currently full.", "Please try a different world");
				return;
			} else if (loginCode == 16) {
				loginScreenPrint("Account permanently disabled.", "Check your message inbox for details");
				return;
			} else if (loginCode == 17) {
	...
	public int blockChatMessages, blockPrivateMessages, blockTradeRequests, blockDuelRequests;
